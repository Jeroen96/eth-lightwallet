...
var cryptoJsParse = require("crypto-js/enc-hex").parse;
var cryptoJsHex = require("crypto-js/enc-hex").Hex;
...
const { add0x, strip0x } = require("./txutils");

const getRandomBuffer = function(size) {
...
KeyStore.prototype.init = function(mnemonic, pwDerivedKey, hdPathString, salt) {
  ...
  if (typeof pwDerivedKey !== "undefined" && typeof mnemonic !== "undefined") {
    ...
    var paddedSeed = leftPadString(mnemonic, " ", 240);
...
KeyStore.createVault = function(opts, cb) {
  ...
  if (!("seedPhrase" in opts)) {
    var err = new Error(
      "Keystore: Must include seedPhrase in createVault inputs."
    );
    return cb(err);
  }
  ...
  KeyStore.deriveKeyFromPasswordAndSalt(opts.password, opts.salt, function(
    err,
    pwDerivedKey
  ) {
...
function generateSalt(byteCount) {
  return getRandomBuffer(byteCount || 32).toString("base64");
}
...
KeyStore.prototype.isDerivedKeyCorrect = function(pwDerivedKey) {
  ...
  return !paddedSeed ? false : true;
};

function encryptString(string, pwDerivedKey) {
  var nonce = getRandomBuffer(nacl.secretbox.nonceLength);
  var encObj = nacl.secretbox(
    naclUtils.decodeUTF8(string),
    nonce,
    pwDerivedKey
  );
  var encString = {
    encStr: naclUtils.encodeBase64(encObj),
...
KeyStore._decryptString = function(encryptedStr, pwDerivedKey) {
  var secretbox = naclUtils.decodeBase64(encryptedStr.encStr);
  var nonce = naclUtils.decodeBase64(encryptedStr.nonce);
  ...
  return naclUtils.encodeUTF8(!decryptedStr ? [] : decryptedStr);
};
...
KeyStore._encryptKey = function(privKey, pwDerivedKey) {
  ...
  encKey = {
    key: naclUtils.encodeBase64(encKey),
    nonce: naclUtils.encodeBase64(nonce)
  };
...
KeyStore._decryptKey = function(encryptedKey, pwDerivedKey) {
  var secretbox = naclUtils.decodeBase64(encryptedKey.key);
  var nonce = naclUtils.decodeBase64(encryptedKey.nonce);
...
KeyStore._computeAddressFromPrivKey = function(privKey) {
  ...
  var pubKeyWordArray = cryptoJsParse(pubKey);
  var hash = SHA3(pubKeyWordArray, { outputLength: 256 });
  var address = hash.toString(cryptoJsHex).slice(24);
...
KeyStore._computePubkeyFromPrivKey = function(privKey, curve) {
  if (curve !== "curve25519") {
    throw new Error(
      'KeyStore._computePubkeyFromPrivKey: Only "curve25519" supported.'
    );
  }

  var privKeyBase64 = new Buffer(privKey, "hex").toString("base64");
  var privKeyUInt8Array = naclUtils.decodeBase64(privKeyBase64);
  var pubKey = nacl.box.keyPair.fromSecretKey(privKeyUInt8Array).publicKey;
  var pubKeyBase64 = naclUtils.encodeBase64(pubKey);
...
KeyStore._concatAndSha256 = function(entropyBuf0, entropyBuf1) {
  ...
  if (totalEnt.length !== entropyBuf0.length + entropyBuf1.length) {
    throw new Error(
      "generateRandomSeed: Logic error! Concatenation of entropy sources failed."
    );
  }
...
KeyStore.generateRandomSeed = function(extraEntropy, entropyStrength) {
  ...
  if (entropy % 32 !== 0 || entropyStrength < 128) {
    throw new Error("ENT", "Values must be ENT > 128 and ENT % 32 == 0");
  }
  if (extraEntropy === undefined) {
    seed = new Mnemonic(getRandomBuffer(entropy / 8), Mnemonic.Words.ENGLISH);
    ...
    throw new Error(
      "generateRandomSeed: extraEntropy is set but not a string."
    );
  }
...
KeyStore.deriveKeyFromPasswordAndSalt = function(
  password,
  salt,
  derkeyLen = 32,
  callback
  ...
  if (!callback && typeof derkeyLen === "function") {
    callback = derkeyLen;
    derkeyLen = 32;
  }
